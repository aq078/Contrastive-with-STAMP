...
Leak-safe version:
  ...
  - windows are generated across full trials, so windows may contain mixed comp/non-comp frames
  - window label uses POSITIVE_FRAME_THRESHOLD, e.g. 30% positive frames => label 1
  - contiguous events/runs are stored as metadata for auditing, but windows are NOT restricted to stay inside one event
...
Output:
  ...
  - "__window_events__" entry holding window key -> touched event/run metadata, useful for audits.
...
EVENT_LABEL_THRESHOLD = 0.5          # only for event/run metadata; does not constrain windowing
...
def aggregate_label(y_window: np.ndarray, agg: str) -> int:
    ...
    Recommended for this script:
      AGGREGATION = "threshold"
      POSITIVE_FRAME_THRESHOLD = 0.30

    That means a window is positive if at least 30% of valid frames
    have comp >= 0.5. Windows are allowed to be mixed.
    """
    ...
    y = y_window[valid]
    comp_frac = float(np.mean(y >= 0.5))
    agg = agg.lower().strip()

    if agg == "threshold":
        return int(comp_frac >= POSITIVE_FRAME_THRESHOLD)
    if agg == "any":
        return int(np.any(y >= 0.5))
    if agg == "majority":
        return int(comp_frac >= 0.5)
    if agg == "mean":
        return int(np.round(np.mean(y)))
    raise ValueError(f"Unknown AGGREGATION='{agg}'")


def window_touched_events(
    window_start_idx: int,
    window_end_idx: int,
    events: List[Dict[str, Any]],
) -> List[Dict[str, Any]]:
    """Return event/run metadata overlapping a trial-window [start_idx, end_idx)."""
    touched = []
    for ev in events:
        overlap_start = max(window_start_idx, int(ev["start_idx"]))
        overlap_end = min(window_end_idx, int(ev["end_idx"]))
        overlap = max(0, overlap_end - overlap_start)
        if overlap > 0:
            touched.append({
                "event_idx": int(ev["event_idx"]),
                "event_label": int(ev["event_label"]),
                "event_frames_start": int(ev["frames_start"]),
                "event_frames_end": int(ev["frames_end"]),
                "overlap_frames": int(overlap),
            })
    return touched


def build_label_events(frames: np.ndarray, y: np.ndarray) -> List[Dict[str, Any]]:
    """
    Split a trial into contiguous movement/background runs for metadata/auditing.

    Important: these events/runs are NOT used to constrain windowing.
    Windows are generated across the full trial so they can contain mixed
    comp/non-comp frames. The subject-disjoint split prevents leakage.

    Returns event dicts with Python slice indices [start_idx, end_idx).
    """
    ...
    def close_event(end_idx: int) -> None:
        nonlocal cur_start, cur_label
        if cur_start is None:
            return
        if end_idx - cur_start >= MIN_EVENT_FRAMES:
            events.append({
                "event_idx": len(events),
                "event_label": int(cur_label),
                "start_idx": int(cur_start),
                "end_idx": int(end_idx),
                "frames_start": int(frames[cur_start]),
                "frames_end": int(frames[end_idx - 1]),
            })
        cur_start = None
        cur_label = None

    prev_frame = None
    for i, (fr, yi) in enumerate(zip(frames, y)):
        if np.isnan(yi):
            close_event(i)
            prev_frame = None
            continue

        label = int(yi >= EVENT_LABEL_THRESHOLD)
        starts_new = (
            cur_start is None
            or cur_label != label
            or prev_frame is None
            or int(fr) != int(prev_frame) + 1
        )
        if starts_new:
            close_event(i)
            cur_start = i
...
def compute_pid_window_counts() -> dict:
    ...
    for ex in EXERCISES:
        ...
        for row in vdf.itertuples(index=False):
            ...
            for start in range(0, T - WINDOW_L + 1, STRIDE):
                ...
                valid = ~np.isnan(yw)
                if valid.sum() > 0:
                    ratios.append(float(np.mean(yw[valid] >= 0.5)))
    ...
    if ratios:
        uniq_ratios = sorted(set(round(r, 3) for r in ratios))
        print("\nUnique window positive ratios:", flush=True)
        print(uniq_ratios[:30], flush=True)
        print(uniq_ratios[-30:], flush=True)
    ...
    with db.begin(write=True) as txn:
        for ex in EXERCISES:
            ...
            for row in vdf.itertuples(index=False):
                ...
                events = build_label_events(frames, y)
                T = X.shape[0]
                for start in range(0, T - WINDOW_L + 1, STRIDE):
                    end = start + WINDOW_L
                    Xw = X[start:end]
                    ...
                    valid = ~np.isnan(yw)
                    positive_frame_fraction = float(np.mean(yw[valid] >= 0.5)) if valid.sum() > 0 else float("nan")
                    touched_events = window_touched_events(start, end, events)
                    ...
                    meta = {
                        ...
                        "affected": affected,
                        "trial": trial,
                        "trial_id": f"{ex}_pid{pid}_aff{affected}_trial{trial}",
                        "frame_init": frame_init,
                        "frame_end": frame_end,
                        "trial_local_start": int(start),
                        "trial_local_end": int(end),
                        "frames_start": int(frames[start]),
                        "frames_end": int(frames[end - 1]),
                        "positive_frame_fraction": positive_frame_fraction,
                        "positive_frame_threshold": POSITIVE_FRAME_THRESHOLD,
                    ...
                    window_events[key] = touched_events
                    kept_windows += 1
        ...
        txn.put(b"__window_events__", pickle.dumps(window_events, protocol=pickle.HIGHEST_PROTOCOL))
