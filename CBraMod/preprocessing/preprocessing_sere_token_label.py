...
"""
#token label:sample: (33, 3, 64)
# temporal_token_comp_labels: (8,)
# token_comp_labels: (792,)   # 8 * 33 * 3

#window: 64 frames, 30 frames per second
...
def frame_to_token_labels(y_window: np.ndarray, patch_len: int = 8) -> np.ndarray:
    token_labels = []

    for start in range(0, len(y_window), patch_len):
        patch = y_window[start:start + patch_len]
        valid = patch[~np.isnan(patch)]

        if len(valid) == 0:
            token_labels.append(-1)
        else:
            positive_ratio = np.mean(valid >= 0.5)

            token_labels.append(
                int(positive_ratio >= PATCH_LABEL_THRESHOLD)
            )

    return np.asarray(token_labels, dtype=np.int64)
        ...
        for ex in EXERCISES:
            ...
            for row in vdf.itertuples(index=False):
                ...
                for start in range(0, T - WINDOW_L + 1, STRIDE):
                    ...
                    temporal_token_comp_labels = frame_to_token_labels(yw, patch_len=8)  # [8]

                    S = Xw_st.shape[0] * Xw_st.shape[1]  # 33 * 3 = 99
                    token_comp_labels = np.repeat(temporal_token_comp_labels, S).astype(np.int64)  # [8 * 99]
                    value = pickle.dumps(
                            {
                                "sample": Xw_st,
                                "label": int(y_agg),
                                "token_comp_labels": token_comp_labels,
                                "temporal_token_comp_labels": temporal_token_comp_labels,
                                "mask": mask,
                                "meta": meta,
                            },
