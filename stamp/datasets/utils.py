...
dataset_params = {
    ...
    'n_temporal_channels': 8,
    'n_samples': 2971,   # fill after counting#32-8456,64-2971,16-5131, 80-1115
...
def get_embeddings_dir(dataset_name, embedding_model_name, datasets_dir):
    ...
    # if embedding_model_name == 'moment':
    #     model_folder ="MOMENT-1-large"
    return 'embeddings' + f'/{dataset_name}/{embedding_model_name}'
...
def load_moment_model(model_name, model_dir, device="cuda:0"):
    """
    model_name: can be a label like 'MOMENT-1-large' (only used for logging)
    model_dir: local path to folder containing the MOMENT weights/config
    """
    import os
    from momentfm import MOMENTPipeline
    ...
    if ckpt_path is None or not os.path.isdir(ckpt_path):
        raise ValueError(f"model_dir must be a local folder. Got: {model_dir}")
    ...
    # IMPORTANT: MOMENT docs require init()
    model.init()
...
def load_tspulse_model(model_name, tspulse_models_dir, **kwargs):
    ...
    if TSPulseForReconstruction is None:
        raise ImportError("TSPulse requested but tsfm_public is not usable in this environment.")   
