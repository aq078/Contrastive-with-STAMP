...
class STAMPAggregationAblation(nn.Module):
    def __init__(
        ...
        assert encoder_aggregation in ["attention_pooling",      # baseline MHAP
    "mean_feature",
    "max_feature",
    "mean_logits",
    "max_logits"], \
        ...
        if self.encoder_aggregation == "attention_pooling":
            assert mhap_params is not None
            self.multi_head_attention_pooling = MultiHeadAttentionPooling(D=D,
                                                    A=mhap_params['A'],
    ...
    def forward(self, x, return_attention, labels=None):
        ...
        elif self.encoder_aggregation == "mean_feature":
            out = tokens.mean(dim=1)
            out = self.classifier(out)

        elif self.encoder_aggregation == "max_feature":
            out = tokens.max(dim=1).values
            out = self.classifier(out)

        elif self.encoder_aggregation == "mean_logits":
            token_logits = self.token_classifier(tokens)
            out = token_logits.mean(dim=1)
