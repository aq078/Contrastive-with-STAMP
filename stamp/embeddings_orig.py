...
try:
    from tsfm_public.models.tspulse.utils.helpers import get_embeddings as tspulse_get_embeddings
except Exception:
    tspulse_get_embeddings = None
...
def embed_single_gpu_worker(data_loader, batch_size, n_temporal_channels, n_spatial_channels,
    ...
    with torch.no_grad():
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            ### DEBUG CHECKS: INPUT START ###
            # Check x_data / mask for NaN/Inf before model forward
            # if torch.isnan(x_data).any() or torch.isinf(x_data).any():
            #     print('BAD INPUT x_data', 'batch_idx', batch_idx,
            #           'nan', torch.isnan(x_data).any().item(),
            #           'inf', torch.isinf(x_data).any().item(),
            #           'shape', tuple(x_data.shape))
            #     continue
            # if not is_chronos:
            #     if torch.isnan(mask).any() or torch.isinf(mask).any():
            #         print('BAD INPUT mask', 'batch_idx', batch_idx,
            #               'nan', torch.isnan(mask).any().item(),
            #               'inf', torch.isinf(mask).any().item(),
            #               'shape', tuple(mask.shape))
            #         continue
            #     # If the mask is entirely False, some models/aggregations can produce NaNs
            #     if (~mask).all():
            #         print('BAD MASK (all False)', 'batch_idx', batch_idx, 'shape', tuple(mask.shape))
            #         mask[:, 0] = True
            #         continue
            ### DEBUG CHECKS: INPUT END ###

            # Forward pass
            if 'moment' in model_name.lower():
                # x_data: (N, 1, L), mask: (N, L)  [bool]
                # Make sure we never feed NaNs into MOMENT
                x_seq = x_data.squeeze(1)  # (N, L)

                finite = torch.isfinite(x_seq)          # True where value is not NaN/Inf
                mask = mask & finite                    # only attend to finite values
                x_seq = torch.nan_to_num(x_seq, nan=0.0, posinf=0.0, neginf=0.0)

                # Put back to (N, 1, L)
                x_data = x_seq.unsqueeze(1)

                # Safety: if a row becomes fully masked, unmask one position (otherwise some models output NaNs)
                ...
                # add this once
                def _tstat(name, t):
                    return (name, tuple(t.shape), str(t.dtype), str(t.device),
                            bool(torch.isnan(t).any().item()), bool(torch.isinf(t).any().item()),
                            float(torch.nan_to_num(t).min().item()),
                            float(torch.nan_to_num(t).max().item()),
                            float(torch.nan_to_num(t).mean().item()))

                fields = []
                for attr in ["reconstruction", "hidden_states", "embeddings", "last_hidden_state"]:
                    if hasattr(outputs, attr):
                        t = getattr(outputs, attr)
                        if torch.is_tensor(t):
                            fields.append(_tstat(attr, t))
                # print("MOMENT tensor stats:", fields[:10]) #debug

                embeddings = getattr(outputs, "embeddings", None)
                if embeddings is None:
                    raise RuntimeError(
                        "MOMENT returned outputs.embeddings=None. "
                        "This usually means the model was loaded in reconstruction/pretraining mode, "
                        "not embedding mode. Fix load_moment_model to use task_name='embedding' + model.init()."
                    )
